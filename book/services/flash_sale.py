...
from book.models import FlashSale, Book, UserBookView
from book.filters import FlashSaleFilter
...
def check_flash_sale(request, book_id):
    ...
    user_viewed = UserBookView.objects.filter(user=request.user, book=book).exists()
    ...
    if user_viewed and upcoming_flash_sale:
        ...
        return Response({
            'message': "No upcoming flash sales for this book."
        })
